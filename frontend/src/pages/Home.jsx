...
function TimelineItem({ item, index }) {
  ...
  return (
    <div ref={ref} style={{ display: "flex", gap: "1.5rem", opacity: visible ? 1 : 0, transform: visible ? "translateX(0)" : "translateX(-20px)", transition: `opacity 0.6s ease ${index * 100}ms, transform 0.6s ease ${index * 100}ms` }}>
      <div style={{ width: "110px", flexShrink: 0, textAlign: "right", paddingTop: "2px" }}>
        <span style={{ fontSize: "11px", color: ACCENT, fontFamily: "monospace", letterSpacing: "0.04em" }}>{item.period}</span>
      </div>
      ...
      <div style={{ paddingBottom: "2rem", flex: 1 }}>
        <h4 style={{ fontFamily: "'Syne', sans-serif", fontSize: "14px", fontWeight: 700, color: TEXT, marginBottom: "0.4rem" }}>{item.title}</h4>
        <p style={{ fontSize: "17px", color: MUTED, lineHeight: 1.7 }}>{item.desc}</p>
      </div>
    </div>
  );
}
...
export default function Home() {
  ...
  return (
    <div style={{ background: BG, color: TEXT, fontFamily: "'DM Sans', 'Segoe UI', sans-serif", minHeight: "100vh", overflowX: "hidden" }}>
      ...
      {/* NAV */}
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, background: "rgba(10,10,15,0.88)", backdropFilter: "blur(16px)", borderBottom: `1px solid ${BORDER}`, padding: "0 4rem", height: "56px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <a href="https://github.com/RohitBoora05" target="_blank" rel="noreferrer" className="btn-primary" style={{ padding: "6px 16px", fontSize: "12px" }}>GitHub</a>
      </nav>
          ...
          <div style={{ display: "flex", alignItems: "flex-start", gap: "2rem", flexWrap: "wrap" }}>
            <div style={{ flex: 1, minWidth: "280px" }}>
              ...
              <p className="hero-fade-3" style={{ fontSize: "18px", color: MUTED, lineHeight: 1.7, maxWidth: "480px", marginBottom: "2rem" }}>
                BTech AI & Data Science · Building cloud infrastructure & studying AI from first principles. Preparing for AWS certification.
              </p>
              <div className="hero-fade-4" style={{ display: "flex", gap: "1rem", flexWrap: "wrap" }}>
                ...
                <span onClick={() => scrollTo("Connect")} className="btn-primary" style={{ cursor: "pointer", borderColor: BORDER, color: MUTED }}>Get in Touch</span>
              </div>
            </div>
            <div className="hero-fade-2" style={{ width: "320px", height: "380px", border: `1px solid ${BORDER}`, borderRadius: "12px", background: BG3, flexShrink: 0, overflow: "hidden", boxShadow: `0 0 40px rgba(124,111,212,0.08)` }}>
              <img src="/profile.jpg" alt="Rohit Boora" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} onError={(e) => { e.target.style.display = "none"; e.target.nextSibling.style.display = "flex"; }} />
        ...
        <RevealSection>
          ...
          <div style={{ background: BG2, border: `1px solid ${BORDER}`, borderRadius: "12px", padding: "2rem 2.5rem", marginBottom: "2rem" }}>
            <p style={{ fontSize: "17px", color: "#B0B0C8", lineHeight: 1.8, marginBottom: "0.5rem" }}>
              I'm a 3rd year student actively learning, building, and exploring. Open to conversations about <em style={{ color: TEXT, fontStyle: "normal" }}>AI safety, cloud engineering, research collaborations,</em> or just interesting problems worth solving.
            </p>
            <p style={{ fontSize: "18px", color: MUTED, lineHeight: 1.7 }}>Reach out — I respond to everyone.</p>
          </div>
      ...
      <footer style={{ borderTop: `1px solid ${BORDER}`, padding: "2rem", textAlign: "center" }}>
        <p style={{ fontSize: "12px", color: MUTED }}>Rohit Boora · Built with React + AWS · <a href="https://github.com/RohitBoora05" target="_blank" rel="noreferrer" style={{ color: ACCENT2, textDecoration: "none" }}>GitHub</a></p>
      </footer>
    </div>
  );
}
